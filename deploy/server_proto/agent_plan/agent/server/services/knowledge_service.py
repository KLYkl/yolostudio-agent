...
class KnowledgeService:
    ...
    def _extract_metric_bundle(metrics: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if isinstance(latest_metrics, dict):
            nested = latest_metrics.get('metrics') if isinstance(latest_metrics.get('metrics'), dict) else latest_metrics
            if isinstance(nested, dict):
                payload = {**payload, **nested}
        if isinstance(payload.get('metrics'), dict):
            payload = {**payload, **dict(payload['metrics'])}
        return payload
    ...
    def _derive_metric_signals(cls, metrics: dict[str, Any] | None) -> tuple[list[str], list[str]]:
        ...
        facts: list[str] = [str(item).strip() for item in payload.get('facts') or [] if str(item).strip()]
        signals: list[str] = [str(item).strip() for item in payload.get('signals') or [] if str(item).strip()]

        run_state = str(payload.get('run_state') or '').strip().lower()
        minimum_facts_ready = bool(payload.get('minimum_facts_ready'))
        ...
        if epoch is None and isinstance(payload.get('progress'), dict):
            epoch = _to_int(payload['progress'].get('epoch'))
        ...
        if precision is None and recall is None and map50 is None and map5095 is None:
            if any(payload.get(key) is not None for key in ('box_loss', 'cls_loss', 'dfl_loss')):
                ...
                facts.append('当前缺少 precision/recall/mAP/loss 等可分析指标')
            if run_state in {'completed', 'stopped', 'failed'} and not analysis_ready:
                if minimum_facts_ready:
                    facts.append('当前只有部分训练事实，建议先补齐评估指标再做判断')
                else:
                    facts.append('当前训练状态可读，但训练结果事实仍不足')
            return cls._dedupe(signals), cls._dedupe(facts)
    ...
    def _derive_comparison_signals(comparison: dict[str, Any] | None) -> tuple[list[str], list[str]]:
        ...
        def _delta(key: str) -> float | None:
            ...
            if isinstance(item, dict):
                return _to_float(item.get('delta'))
            return None

        precision_delta = _delta('precision')
        recall_delta = _delta('recall')
        map50_delta = _delta('map50')
        map_delta = _delta('map')

        quality_delta = map50_delta if map50_delta is not None else map_delta
        if quality_delta is not None:
            facts.append(f'最近训练 mAP 变化={quality_delta:+.4f}')
            if quality_delta >= 0.03:
                signals.append('latest_run_improved')
            elif quality_delta <= -0.03:
                signals.append('latest_run_regressed')

        if precision_delta is not None:
            facts.append(f'最近训练 precision 变化={precision_delta:+.4f}')
        if recall_delta is not None:
            facts.append(f'最近训练 recall 变化={recall_delta:+.4f}')

        if precision_delta is not None and recall_delta is not None:
            if precision_delta > 0.02 and recall_delta < -0.02:
                signals.append('latest_run_more_conservative')
            elif precision_delta < -0.02 and recall_delta > 0.02:
                signals.append('latest_run_more_aggressive')

        left_state = str((payload.get('left_run') or {}).get('run_state') or '').strip().lower()
        right_state = str((payload.get('right_run') or {}).get('run_state') or '').strip().lower()
        if left_state and right_state and left_state != right_state:
            facts.append(f'训练状态变化 {right_state}->{left_state}')
            if left_state == 'failed':
                signals.append('latest_run_failed_after_previous')
            elif left_state == 'completed':
                signals.append('latest_run_completed')
