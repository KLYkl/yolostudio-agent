...
OUTPUT_ROOT="${2:-$HOME/training_real_lifecycle_output/codex_roundtrip}"
DATASET_ROOT="${3:-$HOME/agent_cap_tests/zyb}"
MODEL_PATH="${4:-$HOME/yolov8n.pt}"
EPOCHS="${5:-30}"
TARGET_EPOCH="${6:-2}"
...
export YOLO_TRAIN_MODEL_PATH="$MODEL_PATH"
export YOLO_TRAIN_EPOCHS="$EPOCHS"
export YOLO_TRAIN_TARGET_EPOCH="$TARGET_EPOCH"
export YOLO_TRAIN_STATUS_DELAYS="$STATUS_DELAYS"
export YOLO_TRAIN_EXTRA_POLL_INTERVAL="$EXTRA_POLL_INTERVAL"
export YOLO_TRAIN_EXTRA_POLL_LIMIT="$EXTRA_POLL_LIMIT"

python -m yolostudio_agent.agent.tests.test_zyb_long_training_lifecycle
printf '%s\n' "$YOLO_TRAIN_OUT"
