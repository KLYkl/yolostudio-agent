...
if ($RemoteOutputRoot.StartsWith('$HOME')) {
    $RemoteHome = ($RemoteAppRoot -replace '/[^/]+$', '')
    $RemoteOutputRoot = $RemoteOutputRoot -replace '^\$HOME', $RemoteHome
}
...
Sync-RemoteTextFileNormalized -Server $Server `
    -LocalPath (Join-Path $RepoRoot "deploy\scripts\run_training_remote_validation.sh") `
    -RemotePath "$RemoteAppRoot/deploy/scripts/run_training_remote_validation.sh"
...
Fetch-RemoteFile -Server $Server -RemotePath "$RemoteOutputRoot/remote_training_lifecycle.json" -LocalPath $LocalResultPath
