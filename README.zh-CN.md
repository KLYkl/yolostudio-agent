...
deploy/scripts/             远端验证、roundtrip、bridge 脚本
  deploy/server_proto/        远端部署路径使用的受控运行时镜像
...
- `agent/` 是事实上的源码目录；受控运行时镜像通过 `python deploy/scripts/sync_server_proto.py` 刷新。
- `deploy/server_proto/` 不是手工维护的代码真源。只有 `.venv`、`core/`、`utils/` 和镜像 README 等 deploy-only 路径继续保留在该包内。
