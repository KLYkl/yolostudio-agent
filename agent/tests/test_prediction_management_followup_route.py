...
class _NoLLMGraph:
    ...
    async def ainvoke(self, *args, **kwargs):
        raise AssertionError('prediction management followup route should stay on routed flows, not fallback to graph')
...
async def _scenario_export_followup_routes_from_state() -> None:
    ...
    def _planner_reply(messages) -> str:
        ...
        if '预测结果管理跟进路由器' in text:
            return '{"action":"export","reason":"用户在追问刚才导出的报告"}'
        if '结果说明器' in text:
            return '刚才已经导出了 markdown 报告，文件在 /tmp/predict-out/prediction_summary.md。'
        return '预测报告导出完成。'
...
async def _scenario_path_lists_followup_routes_from_state() -> None:
    ...
    def _planner_reply(messages) -> str:
        text = '\n'.join(str(getattr(message, 'content', message)) for message in messages)
        if '预测结果管理跟进路由器' in text:
            return '{"action":"path_lists","reason":"用户在追问刚才导出的清单"}'
        if '结果说明器' in text:
            return '刚才已经导出了路径清单，目录在 /tmp/predict-out/path_lists。'
        return '预测路径清单导出完成。'
...
async def _scenario_organize_followup_routes_from_state() -> None:
    ...
    def _planner_reply(messages) -> str:
        text = '\n'.join(str(getattr(message, 'content', message)) for message in messages)
        if '预测结果管理跟进路由器' in text:
            return '{"action":"organize","reason":"用户在追问整理后的结果"}'
        if '结果说明器' in text:
            return '整理后的结果目录在 /tmp/predict-out/organized_by_class，一共复制了 2 个文件。'
        return '预测结果整理完成。'
