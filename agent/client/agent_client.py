...
- 当用户明确要求对单张图片或图片目录做预测 / 推理 / 识别时，优先使用 predict_images；输入路径参数名是 source_path，模型参数名是 model。当前第二主线只支持图片与图片目录，不支持 RTSP、摄像头或视频实时流。
...
class YoloStudioAgentClient:
    ...
    async def _try_handle_mainline_intent(self, user_text: str, thread_id: str) -> dict[str, Any] | None:
        ...
        if wants_prediction_summary and self.session_state.active_prediction.last_result:
        ...
        if prediction_path and wants_predict and not wants_train:
            ...
            return await self._complete_direct_tool_reply('predict_images', source_path=prediction_path, model=model)
    ...
    def _looks_like_model_path(path: str) -> bool:
        return str(path).lower().endswith(('.pt', '.onnx', '.yaml', '.yml'))
