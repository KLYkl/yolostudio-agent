...
- 当用户明确要求批量转标注格式时，优先先用 preview_convert_format，再决定是否执行 convert_format；convert_format 会写到新目录，不覆盖原标签。
- 当用户明确要求批量改标签、替换类别、删除某类标签时，优先先用 preview_modify_labels，再决定是否执行 modify_labels；modify_labels 默认会创建 .bak 备份。
- 当用户明确要求清理孤儿标签时，优先用 clean_orphan_labels；默认 dry_run=true，只预览待删文件，只有在确认执行时才会真正删除。
- 当用户明确要求补齐缺失标签、生成空标签时，优先使用 preview_generate_missing_labels / generate_missing_labels 或 preview_generate_empty_labels / generate_empty_labels；这些工具会给出 planned_output_samples，不要凭空编造写入路径。
- 当用户明确要求按类别整理数据、按类别分桶、把含某类目标的图片整理出来时，优先先用 preview_categorize_by_class，再决定是否执行 categorize_by_class；categorize_by_class 只复制到新目录，不改原始数据。
...
class YoloStudioAgentClient:
    ...
    async def _try_handle_mainline_intent(self, user_text: str, thread_id: str) -> dict[str, Any] | None:
        ...
        wants_predict = bool(re.search(r'\b(predict|prediction|infer|inference)\b', normalized_text)) or any(token in user_text for token in ('预测', '推理', '识别'))
        ...
        if wants_prediction_report_export:
            export_kwargs = self._prediction_followup_kwargs(user_text, fallback_path=prediction_path)
        ...
        if wants_prediction_path_lists:
            path_list_kwargs = self._prediction_followup_kwargs(user_text, fallback_path=prediction_path)
        ...
        if wants_prediction_result_organize:
            organize_kwargs = self._prediction_followup_kwargs(user_text, fallback_path=prediction_path)
    ...
    async def _complete_specific_training_run_outcome_analysis_reply(self, run_id: str) -> dict[str, Any]:
        ...
        reply = self._merge_grounded_sections([
            self._build_grounded_tool_reply([('inspect_training_run', inspection)]),
            self._build_grounded_tool_reply([('analyze_training_outcome', result)]),
        ])
        if not reply:
    ...
    def _build_training_plan_draft(
        ...
        *,
        ...
        model_hint = str(
            next_tool_args.get('model')
            or planned_training_args.get('model')
            or ''
        ).strip().lower()
        if any(token in user_text for token in ('视频模型', 'video model', '视频权重', '视频用的权重')) or 'video_model' in model_hint or '/video' in model_hint:
            video_model_warning = '当前训练主线面向图片检测；如果该权重来自视频/时序任务，请先确认任务与权重是否匹配。'
            if video_model_warning not in warnings:
                warnings.insert(0, video_model_warning)
            if video_model_warning not in risks:
                risks.insert(0, video_model_warning)
        ...
        if warnings and any('图片检测' in item and '权重' in item for item in warnings):
            reasoning_parts.append('你提到这是视频模型；当前按图片检测训练前，建议先确认权重任务是否匹配。')
    ...
    async def _try_handle_training_plan_dialogue(self, user_text: str, thread_id: str) -> dict[str, Any] | None:
        ...
        wants_resume_recent_training = any(token in user_text for token in ('从最近状态继续训练', '从最近状态继续', '从最近状态恢复训练', '恢复刚才训练', '接着上次训练', '恢复上次训练', 'resume 上次训练', 'resume 刚才训练', 'resume 另一个 run', 'resume run', '继续另一个 run')) or ('resume' in normalized and ('训练' in user_text or bool(explicit_run_ids)))
        ...
        if contradictory_train_intent:
            ...
            if draft or pending:
                reply = f'{reply}\n\n{self._render_training_plan_draft(draft or {}, pending=bool(pending))}'
            self._messages.append(AIMessage(content=reply))
            return {
