...
## 0.5 2026-04-12 中午新增：训练计划草案的 post-prepare 复杂重规划回归已补上

在 `prepare -> preflight -> start_training` 这段桥接补齐之后，当前又新增了一条更长的复杂回归：
...
这说明训练计划草案确认层已经开始覆盖“prepare 后的复杂重规划”，不再只覆盖短链路 happy path。

## 0.6 2026-04-12 结论补充：训练计划草案确认层首轮收口完成

到当前为止，这一层可以先按“首轮收口完成”理解：

- 训练计划草案已替代早期的工具级确认
- 支持自由讨论、追问原因、修改参数、切执行后端、取消后继续修订
- `prepare -> preflight -> start_training` 已接成连续确认链
- 本地 / proto / 远端都已有对应复杂回归

因此，后续主线可以把重点逐步从“继续补训练计划草案骨架”转向：
...
最近主线推进不是“没测出问题”，而是通过复杂对话测试和远端补验，连续发现并修掉了这些问题：

- 取消确认后计划草案丢失
- “先给我计划”吞掉修订内容
- 显式改环境时被旧环境名误判
- 部分高级参数表达解析不完整
- 长对话里缺少退出当前训练计划并回到数据检查主线的入口

当前已经完成：

- 取消确认后保留训练计划草案
- 支持“不要训练，重新检查能不能直接训练”退出口
- 训练环境解析改进
- 高级参数解析继续补齐
...
> 继续把“训练计划草案确认层”做厚，重点补更长链路、更复杂的多轮修订与最终执行测试。

### 最新补充：训练计划草案已覆盖“切执行后端再切回”场景
